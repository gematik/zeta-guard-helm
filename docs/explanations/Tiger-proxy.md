...
In this project a standlone Tiger proxy is deployed as a reverse proxy to intercept, decrypt and analyse requests that are routed through the proxy
...
In order to route requests through the proxy, the routing targets of the central `Ingress` ressource are modified depending on the `zeta-guard.routeViaTigerProxy` flag.
