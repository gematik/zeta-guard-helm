...
- `make deploy stage=STAGE [namespace=NAMESPACE] [DB_MODE=operator]` – install/upgrade the release with `--rollback-on-failure --timeout 10m`
  ...
  - `DB_MODE=operator` installs the postgresql operator for local environments if desired.
- `make deploy-debug stage=STAGE [namespace=NAMESPACE] [DB_MODE=operator]` – same as deploy with `--debug`
